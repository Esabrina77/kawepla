...
export default function PlanningPage() {
  ...
  const { remaining, loading: limitsLoading, refresh: refreshLimits } = useServicePurchaseLimits();
  ...
  // ⚠️ On compare en heure locale (getFullYear/getMonth/getDate) pour éviter
  // le décalage UTC qui fait que toISOString() renvoie J-1 pour UTC+1
          ...
          <div className={styles.actionButtons}>
            ...
            <button
              ...
              disabled={
                ...
                (!limitsLoading && remaining !== null && (remaining.aiRequests ?? 0) === 0)
              }
              title={
                invitations.filter(inv => new Date(inv.eventDate) >= new Date()).length === 0
            ...
            <div className={styles.calendarGrid}>
              ...
              {Array.from({ length: getDaysInMonth(currentDate) }).map((_, i) => {
                ...
                return (
                  <div
                    key={day}
                    className={`${styles.calendarDay} ${isToday ? styles.today : ''} ${isSelected ? styles.selected : ''} ${dayTasks.length > 0 ? styles.hasTasks : ''} ${isEvent ? styles.eventDay : ''}`}
                    onClick={() => setSelectedDate(date)}
                  >
                    <span className={styles.dayNumber}>{day}</span>
                    {isEvent && <span className={styles.eventDayBadge}>🎉</span>}
              ...
              <div className={styles.selectedDateTasks}>
                ...
                <div className={styles.dailyTasksList}>
                  {isEventDate(selectedDate) && (
                    <div className={`${styles.emptyDailyTasks} ${styles.eventDayMessage}`} style={{ marginBottom: '1rem', padding: '2rem' }}>
                      <span style={{ fontSize: '2.5rem' }}>🥂</span>
                      <p style={{ fontWeight: 700, fontSize: '1.1rem', marginBottom: '0.25rem' }}>
                        C'est le grand jour ! 🎉
                      </p>
                      <p style={{ fontSize: '0.9rem', opacity: 0.9, lineHeight: 1.5 }}>
                        <strong>Kawepla</strong> vous souhaite de profiter pleinement de votre événement. <br />
                        Vivez ce moment inoubliable, tout est prêt !
                      </p>
                    </div>
                  )}
