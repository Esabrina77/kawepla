...
export default function ClientDesignEditorPage() {
    ...
    const initCanvas = async () => {
            if (canvas && currentDesign) {
                ...
                try {
                    const dimensions = await loadKaweplaDesignToFabric(currentDesign, canvas);
                    // Mettre à jour le format uniquement — ne recrée PAS le canvas
                    setFormat({
                    ...
                    setTimeout(() => setStoreLoading(false), 300);
                }
            }
        };
        initCanvas();
    }, [canvas, currentDesign, setStoreLoading, setFormat]);
