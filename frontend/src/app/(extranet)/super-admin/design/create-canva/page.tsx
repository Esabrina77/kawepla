...
export default function CreateCanvaDesignPage() {
  ...
  const setStoreLoading = useEditorStore((state) => state.setLoading);
  ...
  useEffect(() => {
    const initCanvas = async () => {
      if (canvas && currentDesign) {
        ...
        try {
          // Charger le design et récupérer ses dimensions réelles
          const dimensions = await loadKaweplaDesignToFabric(
            currentDesign,
            canvas,
          );

          // Mettre à jour le store pour que l'éditeur s'adapte
          setFormat({
            width: dimensions.width,
            height: dimensions.height,
            name: currentDesign.canvasFormat || "Custom",
          });
        } catch (error) {
          console.error("Failed to init admin canvas:", error);
        } finally {
          setTimeout(() => setStoreLoading(false), 500);
        }
      }
    };
    initCanvas();
  }, [canvas, currentDesign, setFormat, setStoreLoading]);
