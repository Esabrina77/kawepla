...
import { loadKaweplaDesignToFabric } from '@/utils/fabricToKaweplaAdapter';
...
export default function DesignPreview({ design, width = 300, height = 400, className }: DesignPreviewProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Charger via l'adaptateur pour assurer la fidélité
      loadKaweplaDesignToFabric(
        ...
        canvas
          ...
          // Ajuster le zoom pour que le canvas tienne dans le cadre de preview
          const designWidth = design.canvasWidth || 794;
          const designHeight = design.canvasHeight || 1123;
          ...
          canvasInstanceRef.current.setWidth(designWidth * scale);
          canvasInstanceRef.current.setHeight(designHeight * scale);
