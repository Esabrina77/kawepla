...
router.post('/:id/publish', createRateLimiter as RequestHandler, InvitationController.publish);
router.post('/:id/archive', createRateLimiter as RequestHandler, InvitationController.archive);
...
router.post('/:id/guests/preview-import', createRateLimiter as RequestHandler, uploadMiddleware, (req, res, next) => {
  req.body.invitationId = req.params.id;
  GuestController.previewImport(req, res, next);
});
router.post('/:id/guests/bulk-import', createRateLimiter as RequestHandler, uploadMiddleware, (req, res, next) => {
  req.body.invitationId = req.params.id;
  GuestController.bulkImport(req, res, next);
});
router.post('/:id/guests', createRateLimiter as RequestHandler, checkGuestLimit, validateGuestCreation, (req, res, next) => {
  req.body.invitationId = req.params.id;
  GuestController.create(req, res, next);
});
router.post('/:id/guests/send-all', createRateLimiter as RequestHandler, (req, res, next) => {
  req.body.invitationId = req.params.id;
  GuestController.sendAllInvitations(req, res, next);
});

// Routes de modification (protégées)
router.put('/:id', createRateLimiter as RequestHandler, InvitationController.update);
router.delete('/:id', createRateLimiter as RequestHandler, InvitationController.delete);
